#!/usr/bin/env python3
"""
장기 기억 엔진 (Long-term Memory Engine)
...
class LongTermMemory:
    ...
    def retrieve_similar_experiences(self, query: str, top_k: int = 5,
        ...
        if success_only:
            filtered_memories = [m for m in filtered_memories if m.get("success", False)]
        ...
        try:
            ...
            for idx in top_indices:
                if similarities[idx] > 0.1:  # 최소 유사도 임계값
                    memory = filtered_memories[idx].copy()
    ...
    def _rebuild_vector_index(self):
        ...
        # TF-IDF 벡터화
        try:
            self.memory_vectors = self.vectorizer.fit_transform(memory_texts)
        except Exception as e:
    ...
    def _save_index(self):
        ...
        try:
            ...
            index_to_save = {
                ...
                "by_agent": dict(self.index["by_agent"]),
                "by_success": self.index["by_success"],
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "success_rate": len(self.index["by_success"]["true"]) / len(self.memories) if self.memories else 0,
            "patterns_learned": {
                "success": len(self.patterns["success_patterns"]),
                "failure": len(self.patterns["failure_patterns"]),
                "optimization_rules": len(self.patterns["optimization_rules"])
            },
